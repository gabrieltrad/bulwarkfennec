...
if (SpecialWaveType == "JuggerWave") then {
	JuggerWave = true;
	[] execVM "hostiles\JuggerWave.C";
}else {
	JuggerWave = false;
};
...
if (JuggerWave) then {
	["SpecialWarning",["Juggernaut"]] remoteExec ["BIS_fnc_showNotification", 0];
	["Alarm"] remoteExec ["playSound", 0];
};
