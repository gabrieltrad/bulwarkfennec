...
if (!SUPPORTMENU) then {
	...
	[_satSupport, ["<t color='#ff00ff'>" + "Desbloquear menu de Suporte", "
		...
		if (!SUPPORTMENU) then {
			['TaskAssigned',['Support','Suporte desbloqueado no Bulwark']] remoteExec ['BIS_fnc_showNotification', 0];
...
/* Supply Drop */
planes = ["CUP_B_UH60M_Unarmed_US", "B_T_VTOL_01_vehicle_F", "O_Heli_Transport_04_covered_F"];
planesel = selectRandom planes;
[bulwarkCity, ["<t color='#00ff00'>" + "FILL AMMO", "supports\ammoDrop.sqf","",2,true,false,"true","true",4], planesel] remoteExec ["supports_fnc_supplyDrop", 2];
